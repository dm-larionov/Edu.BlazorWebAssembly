...
namespace Edu.BlazorWebAssembly.Client.Infrastructure.Auth.AzureAd;
...
internal class AzureAdClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
    ...
    public override async ValueTask<ClaimsPrincipal> CreateUserAsync(RemoteUserAccount account, RemoteAuthenticationUserOptions options)
    {
        ...
        if (principal.Identity?.IsAuthenticated is true)
        {
            ...
            if (!userIdentity.HasClaim(c => c.Type == EduClaims.Fullname))
            {
                userIdentity.AddClaim(new Claim(EduClaims.Fullname, $"{userDetails.FirstName} {userDetails.LastName}"));
            }
            ...
            if (!string.IsNullOrWhiteSpace(userDetails.ImageUrl) && !userIdentity.HasClaim(c => c.Type == EduClaims.ImageUrl) && userDetails.ImageUrl is not null)
            {
                userIdentity.AddClaim(new Claim(EduClaims.ImageUrl, userDetails.ImageUrl));
            }

            var permissions = await _services.GetRequiredService<IPersonalClient>().GetPermissionsAsync();

            userIdentity.AddClaims(permissions.Select(permission => new Claim(EduClaims.Permission, permission)));
        }
