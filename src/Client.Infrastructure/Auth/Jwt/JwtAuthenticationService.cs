...
namespace Edu.BlazorWebAssembly.Client.Infrastructure.Auth.Jwt;
...
public class JwtAuthenticationService : AuthenticationStateProvider, IAuthenticationService, IAccessTokenProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        {
            claimsIdentity.AddClaims(cachedPermissions.Select(p => new Claim(EduClaims.Permission, p)));
        }
    ...
    public async Task<bool> LoginAsync(TokenRequest request)
    {
        ...
        var roles = (await _personalClient.GetRolesAsync()).Where(x => x.Enabled).Select(x => x.RoleName).ToArray();

        foreach (var role in roles)
            permissions.Add(role);

        await CachePermissions(permissions);
        await CacheRoles(roles);
    ...
    private ValueTask CacheRoles(ICollection<string> roles) =>
        _localStorage.SetItemAsync(StorageConstants.Local.Roles, roles);
    ...
    private async Task ClearCacheAsync()
    {
        ...
        await _localStorage.RemoveItemAsync(StorageConstants.Local.Roles);
    }
