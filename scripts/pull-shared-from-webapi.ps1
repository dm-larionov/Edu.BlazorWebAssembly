...
$sourcePath = Join-Path -Path $rootDirectory -ChildPath '..\dotnet-webapi\src\Core\Shared'
$destinationPath = Join-Path -Path $rootDirectory -ChildPath 'src\Shared'

$excludes = @('bin', 'obj')

Write-Host "Pull changes from the Education Process WebApi Shared Project"
write-Host "---------------------------------------------------------"
Write-Host
